...
public class CafeteriaDoor : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Open)
        {
            Quaternion DoorRotation = Quaternion.Euler(-90, DoorOpen, 90);
            transform.localRotation = Quaternion.Slerp(transform.localRotation, DoorRotation, OpenCloseSpeed);
        }
        else
        {
            Quaternion DoorRotation2 = Quaternion.Euler(-90, DoorClosed, 90);
